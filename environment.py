...
class Node:
    ...
    def estimated_finish(self, task=None) -> float:
        work = self.workload()
        if task:
            work += task.size
    ...
    def _process(self, task: Task) -> Dict:
        ...
        self.task_history.append({
            'task_id': task.id, 'start': start, 'finish': finish,
            'latency': latency, 'miss': is_miss, 'deadline': task.deadline
        })
...
class Environment:
    ...
    def generate_tasks(self, n_tasks: int, regime: str = "MIXED"):
        tasks = []
        t = 0.0
        for i in range(n_tasks):
            ...
            tasks.append(Task(id=i, arrival=t, size=max(size, 0.01), deadline=deadline, priority=priority))
        return tasks
